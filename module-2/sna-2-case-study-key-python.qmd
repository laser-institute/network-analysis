...
print(nx.to_pandas_edgelist(dlt1_network))
...
# Clean column names
dlt2_ties.columns = dlt2_ties.columns.str.lower().str.replace(' ', '_')

# Display the DataFrame
print(dlt2_ties)

# Read dlt2-nodes csv
dlt2_actors = pd.read_csv("data/dlt2-nodes.csv",
                          dtype={
                              'UID': 'str',
                              'Facilitator': 'str',
                              'expert': 'str',
...
# Clean column names: convert to lowercase and replace spaces with underscores
dlt2_actors.columns = dlt2_actors.columns.str.lower().str.replace(' ', '_')

# Display the DataFrame
print(dlt2_actors)

# Create a MultiDiGraph from the dlt2_ties pandas DataFrame
dlt2_network = nx.from_pandas_edgelist(
    dlt2_ties,
    source='sender',
    target='reciever',
    edge_attr=True,
...
print(nx.to_pandas_edgelist(dlt2_network))
```
...
Degree centrality values are normalized by dividing each node's degree by n−1n-1n−1, where n is the total number of nodes in the dlt1_network. Because dlt1_network is a multidigraph with self-loops, the maximum degree can exceed n−1n-1n−1, resulting in degree centrality values that can be greater than 1.
...
# Convert the directed graph 'dlt1_network' to an undirected graph 'simple_graph'
simple_graph = nx.Graph(dlt1_network)
# Calculate the transitivity of the undirected graph
transitivity = nx.transitivity(simple_graph)
#Display the transitivity value
print(transitivity)
...
# Extract largest strongly connected component and create its subgraph
...
print("Number of nodes:", dlt2_network.number_of_nodes())
# Display the number of edges in the 'dlt2_network' graph
print("Number of edges:", dlt2_network.number_of_edges())
# Display the size of the graph in terms of edges,the same as the number of edges
print("Graph size (edges):", dlt2_network.size())

# Display the degree of each node in the network
print("Node degrees:", dict(dlt2_network.degree()))
# Display the density of the graph
print("Density:", nx.density(dlt2_network))
# Display the reciprocity of the graph
print("Reciprocity:", nx.reciprocity(dlt2_network))
# Display the transitivity of the graph
print("Transitivity:", nx.transitivity(nx.Graph(dlt2_network)))

# Extract the largest strongly connected component and create its subgraph
largest_scc = max(nx.strongly_connected_components(dlt2_network), key=len)
largest_scc_graph = dlt2_network.subgraph(largest_scc)

# Calculate and display the diameter and mean distance
print("Diameter:", nx.diameter(largest_scc_graph))
print("Average shortest path length:", nx.average_shortest_path_length(largest_scc_graph))
```
