...
package, the Statnet Team has developed a suite of R packages for the
...
in {statnet} relies on Exponential-family Random Graph Models (ERGMs).
...
full functionality of `igraph` in a tidy API giving you access to almost
all of the `dplyr` verbs plus a few more, developed for use with
relational data.
...
of the **collaboration** network for year 3 is shown in Figure 9.1
copied below.
...
589 (note this could be any number) so we produce the same results each
time.
