...
As noted in the Getting Started lab, exploratory data analysis involves the processes of describing your data (such as by calculating the means and standard deviations of numeric variables, or counting the frequency of categorical variables) and, often, visualizing your data prior to modeling.
...
a.  **Examine Basic Descriptives**. We focus primarily on actors and edges in this lab, including the edges wights we added in the previous section as well as node degree, and import and fairly intuitive measure of centrality.

b.  **Make a Sociogram**. Finally, we wrap up exploratory analysis by learning to plot a network and tweak key elements like the size, shape, and position of nodes and edges to better at communicating key findings.
...
# And again, ignore the Error in attrs message. This won't impact our analysis. 
```

**RStudio Tip:** We have another reason for reviewing and running our code from the previous lab. Each R Markdown document is a self-contained file. In order to be able to knit a share a markdown file, all code required to run an analysis must be present in the document even if those objects and packages are loaded and saved in your environment.
...
> Degree is the number of ties to and from an actor. In a directed network, in-degree is the number of ties received, whereas out-degree is the number of ties sent.
...
**Note**. We set the `mode =` argument in this function to "all" which gives us the total number of participants that each actor in our network sent or received a reply.

Let's take a look at the distribution of `node_degree` in our network by using R's built in `hist()` function for creating histograms. We set the value of `breaks =`, or bins in our histogram, to 30 since I already know some actors in this network have a very high degree.
...
Use the code chunk below to create a histogram and calculate the mean and median from the edge weights created earlier by using the `edge_weights` data frame, the `$` operator, and column `n` which contains the counts for each unique edge.

The function has been provided for you but you will need to supply the appropriate argument. **Hint:** Every thing you need is supplied in the above paragraph.
...
There are many other [igraph layout functions](https://igraph.org/r/doc/layout_.html), some more useful than others depending on the context. We've only scratched the surface of what's possible with network plotting but hopefully this have given you a sense of what is required to avoid the dreaded "hair ball" plot and create a network viz that communicates something useful.
...
Recall that our guiding study, A Social Network Perspective in MOOC-Eds, investigated the following questions:
...
Finally, if you're interested interested in learning more about ERGMs and selection processes in networks, highly recommend the very entertaining and informative introduction to ERGMs by Benjamin Lind, [Lessons on exponential random graph modeling from Grey's Anatomy hook-ups](https://badhessian.org/2012/09/lessons-on-exponential-random-graph-modeling-from-greys-anatomy-hook-ups/).
...
In this learning lab, we learned some new functions for retrieving basic network descriptives related to our research question and used some of these metrics to create and refine a network visualization that helps illustrate key findings. Below, add a few notes in response to the following prompts.
...
Congratulations - you've completed SNA Module 1: The Social Network Perspective and MOOC-Eds! To complete your work, you can click the drop down arrow at the top of the file, then select "Knit top HTML". This will create a report in your Files pane that serves as a record of your code and its output you can open or share.

## Reach (Optional)

We've only scratched the surface of network visualization. For you reach, we encourage you to try out some of the other graphing options from from Katya Ognyanova's tutorial on [Static and Dynamic Network Visualization with R](https://kateto.net/network-visualization/).
