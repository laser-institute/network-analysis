...
### Network Theory
- Recall that in previous labs we focused on ways in which data visualization and network measurement can be used to describe properties of complete networks, groups within a network, or even indidviduals within a network.  
...
- One downside to relying just on visualization and descriptive measures is that they convey the image of a fixed network with information or behaviors passing through it, much like airplanes flying to and from different airports.
- In reality, unlike airports, networks change, sometimes very frequently. If you've ever taught in a classroom, you know students friendships can be in a constant state of flux. Teachers collaborate with new colleagues all the time and no longer do so with old ones.
- In addition, properties of these representations of networks 
- In Part 1, you'll be introduced to about the ways in which statistical inference has evolved in order to move social network analysis beyond the social scientific goal of description and closer to explanation and ultimately prediction.
...
.panel[.panel-name[Beyond Description]

move social network analysis beyond the social scientific goal of description and closer to explanation and ultimately prediction.

 
- we need a way to simultaneously analyze changes in networks and behaviors
- a different statistical approach was needed that did not assume the independence of a network's actors. 
...
.pull-left[
- Network data violates two core assumptions. Guesses?
- Enter the permutation (and other nonparametric tests). 
- Matrices are rearranged over and over (even thousands of times!)
- Permutations compared to your data      
- If often similar, likely due to chance.
]
...
.panel[.panel-name[Research Questions]
