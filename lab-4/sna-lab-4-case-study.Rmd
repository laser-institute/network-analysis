---
title: 'SNA Case Study 4: Birds of a Feather Lead Together'
...
**SNA Case Study: Birds of a Feather Lead Together** is guided once
again by the work of [Alan
...
\<\<\<\<\<\<\< HEAD:keys/sna-selection-case-study-key.Rmd package, the
Statnet Team $$\@handcock:statnet$$ has developed a suite of R packages
for the management, exploration, statistical analysis, simulation and
vizualization of network data. The statistical modeling framework used
in {statnet} relies on Exponential-family Random Graph Models.
...
of the **Year 3 [Collaboration]{.underline} Network** shown in Figure
9.1 copied below. Each directed line represents a collaboration tie
between school leaders. Female school leaders are represented circles
and males by square nodes; and nodes of district-level leaders are
clear, while school-level leaders are filled in.
...
3 [Confidential]{.underline} Exchange Network** by modifying the network
graph template code below and tweaking the included function/arguments
...
package, however, data will need to be transformed into a `network`
object --- the format that statnet uses to store and work with network
data.
