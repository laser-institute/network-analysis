---
title: 'SNA Case Study 4: School Leader Selection'
subtitle: "LASER Institute Learning Labs"
author: "Dr. Shaun Kellogg"
date: "November 16, 2021 (updated: `r format(Sys.Date(),'%B %e, %Y')`)"
output:
  html_document:
    toc: yes
    toc_depth: 4
...
**SNA Case Study: School Leader Selection** is guided by [Alan
Daly](https://scholar.google.com/citations?user=zgbarXQAAAAJ&hl=en) and
colleagues and attempts to replicate the analyses described in Chapter
9: Network Data and Statistical Models from *Social Network Analysis and
Education* [@carolan2014]. In this case study we'll revisit the visual
and mathematical descriptions of networks explored in previous case
studies to help uncover, and then empirically test, generative processes
that attempt to explain how school leaders select peers for
collaboration or confidential exchanges.
...
1.  **Prepare**: Prior to analysis, we'll take a look at the context
    ...
    introduce the {statnet} R package for ERGMs.
...
5.  **Communicate**: We briefly reflect on our case study in preparation
    for our independent analysis next week.
...
In response to research questions 1 & 2 from above, Carolan reported
...
For our Unit 3 case study, we focus specifically on generative
...
it by the end of this case study.
...
using them again in this case study:
...
<<<<<<< HEAD:keys/sna-selection-case-study-key.Rmd
package, the Statnet Team $$\@handcock:statnet$$ has developed a suite
of R packages for the management, exploration, statistical analysis,
simulation and vizualization of network data. The statistical modeling
framework used in {statnet} relies on Exponential-family Random Graph Models.
...
section of our case study for bullets 1-3 above:
...
2.  [**School Leaders Data Chapter
    ...
    variables will be used as covariates in our ERGMs later in this case
    study.
...
To date in these learning labs, we have only worked with edge-lists as a
data format for storing network data. Recall that edge-lists contain a
row for each dyad consisting of at minimum two columns with the name of
each actor, and which conveniently can also contain other information or
...
### **Convert to Network Graph Object**

Recall from Learning Lab 1 that we introduced the {tidygraph} package
...
Similar to Lab 1, we'll use the `tbl_graph()` function to convert our
...
Run the following code to create our `leader_graph` network object which
we'll use to examine descriptive statistics for our network as well as
create network visualization in the EXPLORE section that follows:
...
a.  **Examine Basic Descriptives**. We focus primarily on actors and
    edges in this case study, including whether or not ties were
...
section 3a, we will try to reproduce these descriptives.
...
which is a little lower than that reported by Carolan. Again, this is
...
How close are your results to those reported by Carolan Table 9.1?

-   YOUR RESPONSE HERE

### Add Edge Reciprocity

Before proceeding to our network visualization, there is one important
piece of information that we want to capture about our edges and that
will factor into our modeling for Part 4. Specifically, we are
interested in identifying which confidential exchanges flow both ways,
or in SNA terminology, whether a tie mutual or reciprocated.

**Reciprocity** is a well studied social norm that has been acknowledged
by ancient writers such as Cicero, who stated:

> There is no duty more indispensable than that of returning a kindness.

Not surprisingly, studies of network processes in education have
demonstrated that reciprocity is an important factor in both student and
teacher relations. For example, using statistical techniques that we'll
apply later in this case study, researchers have found that the presence
of reciprocated ties in adolescent friendship networks is significantly
greater than would be expected by chance (Goodreau, Kitts, & Morris,
2009; Lubbers & Snijders, 2007). Using data from the National
Longitudinal Study of Adolescent Health, a nationally representative
dataset of youth, Vaquera and Kao (2008) also found that adolescents
with reciprocated friendships report higher levels of school belonging
and that reciprocity and school belonging both exert independent effects
on academic performance.

Similar to the process for adding network attributes to nodes
demonstrated above, the `activate()` and `mutate()` functions from the
{tidygraph} package can be combined to add new edge attributes to our
network object.

Run the following code to create a new variable for each edges that
indicates whether a confidential exchange is reciprocated or whether the
tie between those two individuals is "asymetric" or not reciprocated.

```{r}
leader_network <- leader_measures %>%
  activate(edges) %>%
...
Use the code chunk below to create an `edge_measures` data frame similar
to one we created above for `node_measures`. Then take a quick `count()`
of the values in our newly created `mutual` column to answer the
...
1.  How many confidential exchanges, i.e. edges in our school leader
    network are reciprocated?

    -   YOUR RESPONSE HERE

2.  How many edges in our network are aasymetric?
...
of the **Year 3 [Collaboration]{.ul} Network** shown in Figure 9.1
copied below. Each directed line represents a collaboration tie between
school leaders. Female school leaders are represented circles and males
by square nodes; and nodes of district-level leaders are clear, while
school-level leaders are filled in.

![](img/figure-9-1.jpg){width="90%"}

At the center of the graph are four male district-level leaders that
send and receive a large number of ties. Carolan notes that, "Given that
males constitute 44% of the sample, this "central" location is
noteworthy."

Note also that the right side of the graph consists mainly of
school-level leaders who tend collaborate among themselves, while the
left side of the graph shows a similar pattern among district-level
leaders.

Finally, unlike our confidential exchange network that we'll look at in
just a second, not one of the leaders in this network is isolated.

Examining network descriptives and sociograms can help us ask a number
of questions about our network, like whether the tendency for district-
and school-level leaders to collaborate among themselves is expected and
"normal" in this network configuration, or how one network position
(e.g. centrality) might be associated certain attributes of nodes (e.g.
male). In our modeling section later in this case study, we'll examine
approaches for statistical inference in networks that can help us answer
these questions more rigorously than our network descriptives and
sociograms.

### 👉 Your Turn **⤵**

Try creating a similar, or ideally even better, sociogram for the **Year
3 [Confidential]{.ul} Exchange Network** by modifying the network graph
template code below and tweaking the included function/arguments or
adding new ones for
...
2.  What patterns in our network, if any, do you observe among leaders'
    confidential exchanges that might warrant more rigorous
    investigation?
...
As we've discovered in these learning labs, network data can come in
...
objects; attributes for the nodes, ties and dyads in can also various
forms and added in various ways to objects in R. For the {ergm} package,
however, data will need to be transformed into a `network` object ---
the format that statnet uses to store and work with network data.
...
and 36 reciprocated dyads. These reciprocated dyads are consistent with
our previous counts of reciprocated edges. Recall from above that 72
reciprocated edges, and since each leader occurs as both a sender and
receiver, that means we have 36 unique pairs of leaders, or dyads.

Since the `ergm()` function automatically uses a stochastic MCMC-based
estimation algorithm, we need to use the `set.seed()` function and set
the value to 589 (note this could be any number) so others produce the
same results each time.

Now let's estimate our first model, save the results as `ergm_mod_1`,
and use the `summary()` function again to take a look at our estimates.
Also,
...
indicating the importance of each parameter provided to the presence of
a tie, estimated coefficients can be explained in terms similar to
logistic regression. That is, positive significant coefficients indicate
that the corresponding parameters in the observed network (e.g.
reciprocated ties between school leaders, controlling for all other
parameters in the model, occur more than would be expected by chance,
thus increasing the likelihood that a tie will occur, and vice-versa for
...
parameter, as noted by @carolan2014, implies that the probability of a
confidential exchange tie in year 3 is relatively low. The reciprocity
parameter, on the other hand, is 3.12 in our model, and indicates a
strong tendency for confidential-exchange ties to be reciprocated. This
is not surprising since we discovered that nearly half of our
confidential exchanges were reciprocated.

Now let's add one other standard ergm-term to capture transitivity, or
the "friend of a friend" phenomenon that is very typical in social
networks. Ergm-terms for transitivity such as `triangles` and
`transitive` in particular are prone to model degeneration, i.e. when
models that fail to converge by reaching a value expected by the
parameters included in the model. Since modeling transitivity is both
computationally intensive because of the all the possible "triangles" in
a directed network, and also very prone to model degeneration, we will
use a "more robust way of modeling triangles: the geometrically-weighed
edgewise shared partner term (GWESP)." a special `gwesp` term which
notes that this a "more robust way of modeling triangles: the
geometrically-weighed edgewise shared partner term (GWESP)."

Let's add the `gwesp` term from the {ergm} package to our model with the
suggested defaults from the Statnet Tutorial section on [What it looks
like when a model
...
estimates. **Note that this make take a couple minutes to run.** If
you'd like to watch the number of iterations of the model set
`message = TRUE`.
...
there is a tendency toward transitivity, i.e. a confidential exchange is
likely to occur between two people who both have a confidential exchange
...
Sadly, we still were not able to replicate the findings from
@carolan2014, but we also did not replicate their model exactly, nor did
we fine tune our model parameters. Regardless, this provided a practical
exercise for demonstrating ERGMs and hopefully made more sophisticated
approaches to statistical inference like ERGMs seem slightly more
approachable.

### **👉 Your Turn** **⤵**

Before moving on to check the goodness of fit for our final model, try
testing a new model by modifying or adding your own ergm-term. For
...
leaders are more likely to confide in colleagues of their own gender, or
as we saw above, colleagues at the same school- or district-level.
...
Congrats! You've finished the Unit 3 Guided case study and are ready for
your independent analysis next week!
